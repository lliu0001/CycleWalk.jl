...
import Pkg
Pkg.activate("Users/lliu001/Desktop/CycleWalk") #Changed from Pkg.activate("CycleWalk_env"; shared=true)
...
num_dists = 5 #Number of districts 
...
pctGraphPath = "/Users/lliu001/Desktop/CycleWalk.jl/examples/data/ct/CT_pct20.json"
nodeData = Set(["COUNTY", "NAME", "POP20", "area", "border_length"]); #Encode data of each node (corresponding to a precinct/county, etc.)
graph = build_graph(pctGraphPath, "POP20", "NAME", nodeData; #Specifies parameters for node weight, label, & metadata (nodeData)
              area_col="area", node_border_col="border_length", #Specify which column to use for each parameter (includes perimeter of each node/county)
              edge_perimeter_col="length") #Each edge connects two adjacent counties
...
output_file_path = joinpath("/Users/lliu001/Desktop/CycleWalk.jl/output","ct", atlasName) # add output directory to path
