...
function get_node_index_to_dist(partition::MultiLevelPartition)
    base_graph = partition.graph.graphs_by_level[end]
    node_ind_to_dist = Vector{Int}(undef, base_graph.num_nodes)

    for di = 1:partition.num_dists
        dg = partition.subgraphs[di].graphs_by_level[1][()]
        vmap = partition.subgraphs[di].vmaps[1][()]
        for node_ind in vmap
            node_ind_to_dist[node_ind] = di
        end
    end
    return node_ind_to_dist
end
